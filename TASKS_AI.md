...
- 同一份历史表支持自动分流：正式订单继续导入 `Order`，预定单可按“订单类型/单据类型”或状态自动导入 `Reservation`
- 预定单导入时会沿用产品管理中的产品名称匹配，并将“预收押金”写入预定单订金
...
- 小程序认证服务
- 小程序 API 测试
- 小程序登录现在会保存微信客户基础档案：`openid / unionid / 昵称 / 头像`
- 客户在小程序提交意向订单时，会自动把手机号、微信号回写到 `WechatCustomer`
- 小程序下单页已支持微信手机号一键授权，后端通过微信官方手机号接口换取手机号并同步到 `WechatCustomer`
- 后台系统已新增“微信客户档案”页面，可按昵称/手机号/微信号/资料完整度查看微信授权用户，并查看历史用户自动补全规则
- 微信客户档案页现已支持后台手工编辑与删除；删除客户时如存在员工绑定，会一并解除绑定
- 微信客户档案页现已支持批量操作：批量删除、批量停用、批量导出选中客户资料
...
- 已调整本地启动脚本：`start.ps1` 默认不再自动执行 `scripts/init_data.py`，避免测试环境每次启动都灌入演示账号、SKU 与部件；如确需初始化演示数据，需显式传 `-InitDemoData`
